...
func Analyze(tracks []*tracks.Track) AnalyzeStruct {
	...
	response.Energy.Median /= float32(len(tracks))
	response.Acousticness.Median /= float32(len(tracks))
	response.Instrumentalness.Median /= float32(len(tracks))
	response.Popularity.Median /= float32(len(tracks))
	response.Tempo.Median /= float32(len(tracks))
	return response
}
