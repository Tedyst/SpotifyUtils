...
// Stolen from the original go-swagger Implementation to change the base template
...
func SwaggerUI(opts SwaggerUIOpts, next http.Handler) http.Handler {
	...
	return http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		if r.URL.Path == pth {
			copyOpts := opts
			copyOpts.CSRFToken = csrf.Token(r)
			buf := bytes.NewBuffer(nil)
			_ = tmpl.Execute(buf, &copyOpts)
			b := buf.Bytes()
    ...
    window.onload = function() {
      ...
      const ui = SwaggerUIBundle({
		...
		requestInterceptor: (req) => {
			req.headers['{{ .CSRFTokenHeader }}'] = "{{ .CSRFToken }}";
			return req;
		}
      })
